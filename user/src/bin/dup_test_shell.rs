            ...
            if !valid {
                ...
                for (i, process_argument) in process_arguments_list.iter().enumerate() {
                    ...
                    if pid == 0 {
                        ...
                        // redirect input
                        if !input.is_empty() {
                            let input_fd = open(input.as_str(), RDONLY);
                            if input_fd == -1 {
                        ...
                        // redirect output
                        if !output.is_empty() {
                            let output_fd = open(output.as_str(), CREATE | WRONLY);
                            if output_fd == -1 {
                        ...
                        let args_test: Vec<_> = args_copy.iter().map(|arg| arg.as_str()).collect();
                        // execute new application
                        if exec(args_copy[0].as_str(), &args_test.as_slice()) == -1 {
