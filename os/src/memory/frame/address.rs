...
unit_test!(test_phys_page_num, {
    let ppn = PhysPageNum(0x81000);
    utest_assert!(ppn.addr().0 == 0x81000_000, "Addr doesn't match");
    let bytes = ppn.get_bytes_array();
    for byte in bytes {
        *byte = u8::MAX;
    }
    let first_usize = ppn.get_mut::<usize>();
    utest_assert!(*first_usize == usize::MAX, "Read / write failed");
    Ok("passed")
});

unit_test!(test_virt_page_num, {
